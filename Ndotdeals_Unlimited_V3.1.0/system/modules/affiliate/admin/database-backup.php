...
/** database backup **/
	...
	if ("server" == $_POST['savetype'])
	{
		...
		if (!$fd = fopen($sqlfile, 'w'))
		{
			...
			set_response_mes(1, "Cannot open file {$sqlfile}!"); 
		...
		else if (fwrite($fd,$sql) === FALSE)
		{
			...
			set_response_mes(1, "Cannot write to file {$sqlfile}!"); 
			header("Location: database-backup.php");
		}
		else
		{
			//$msg = "Table {$_POST['tbl']} successfully dumped into file {$sqlfile}";
			set_response_mes(1, "Table {$_POST['tbl']} successfully dumped into file {$sqlfile}"); 
			header("Location: database-backup.php");
			$type = 'error';
			fclose($fd);
		}
...
<fieldset class="field" style="margin-left:10px;">         
        <legend class="legend">BACKUP INFORMATION</legend>	
<form action="database-backup.php" method="post" name="adminForm">
		...
		<tbody><tr>
			<td style="width: 40%;">
				...
				<tbody>
				<tr>
					<th colspan="2"><label class="fl"><?php echo $gXpLang['backup_MySQL_database']; ?></label></th>
				</tr>
				<tr>
					<td style="text-align:left;"><strong><?php echo $gXpLang['choose_operation']; ?>:</strong></td>
				</tr>
				<tr>
				...
				<tr>
					<td><input type="submit" name="backup" value="<?php echo $gXpLang['backup']; ?>" /></td>
		...
		<table class="adminform">
			<tbody>
				<tr>
					<th colspan="2"><label class="fl"><?php echo $gXpLang['about_database_backups']; ?></label></th>
				</tr>
				<tr>
