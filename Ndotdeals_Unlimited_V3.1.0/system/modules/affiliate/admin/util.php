...
for($i=0;$i<count($params);$i++)
{
	...
	switch ($params[$i]['type'])
	{
		...
		case "text":
                        if($params[$i]['name'] == "payout_balance" || $params[$i]['name'] == "payout_percent")
                        {
			$txt  = '<tr bgcolor="'.$bgcolor.'">';
			$txt .= "<td width=\"185\">{$params[$i]['description']}</td>";
			$txt .= '<td><input class="text_area required digits" maxlength="2" type="text" name="param['.$params[$i]['name'].']" size="50" value="'.$params[$i]['value'].'" />';
			...
			$txt  = '<tr bgcolor="'.$bgcolor.'">';
			$txt .= "<td width=\"185\">{$params[$i]['description']}</td>";
			$txt .= '<td><input class="text_area required" type="text" name="param['.$params[$i]['name'].']" size="50" value="'.$params[$i]['value'].'" />';
			$txt .= '<a href="javascript: void(0);" onMouseOver="return overlib(\''.$params[$i]['hint'].'\', BELOW, RIGHT);" onmouseout="return nd();" ><img src="'.$gXpConfig['xpurl'].'includes/js/ThemeOffice/tooltip.png" border="0" /></a></td>';
			$txt .= "</tr>";
			echo $txt;
			break;
		...
		case "select":
			...
			$txt .= '<td><select name="param['.$params[$i]['name'].']" class="inputbox required" size="1">';
