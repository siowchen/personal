...
*/
	...
	*/
	...
	function addFund($affId,$com,$sales,$tc,$ts,$rasing)
	{
		$sql = "INSERT INTO `{$this->mPrefix}fund_raising` ";
		$sql .= "(`aff_id`, `sales`,`commission`,`total_sales`,`total_commission`,`raising_amount`, `date`) ";
	        $sql .= "VALUES('{$affId}','{$sales}', '{$com}','{$ts}','{$tc}', '{$rasing}', NOW())"; 
	...
	function updateFundRequested($com)
	{
		...
		$sql = "UPDATE `{$this->mPrefix}sales` set fund_requested = 1 ";
...
/*
	function addFundUpdated($affId,$com,$sales,$tc,$ts,$rasing,$com)
	{
		$sql = "INSERT INTO `{$this->mPrefix}fund_raising` ";
		$sql .= "(`aff_id`, `sales`,`commission`,`total_sales`,`total_commission`,`raising_amount`, `date`) ";
	        $sql .= "VALUES('{$affId}','{$sales}', '{$com}','{$ts}','{$tc}', '{$rasing}', NOW())"; 
...
*/
	...
	* @return int
	*/
	function checkAffiliateByPEmail($pEmail)
	{
		$sql = "SELECT `id` ";
		$sql .= "FROM `{$this->mPrefix}affiliates` ";
		$sql .= "WHERE `paypal_email` = '{$pEmail}'";
	...
	*/
	...
	function getDeals($aCity='',$aLimit='')
	{
		$sql  = "SELECT * FROM coupons_coupons WHERE `coupon_status` = 'A' AND  coupon_startdate <= now() AND coupon_enddate >= now()";
		$sql .= $aCity? " AND `coupon_city` = '{$aCity}' " : ""; 





		$sql .= " AND coupon_enddate > now()";
	...
	function getDeal($aId)
	{
		$sql  = "SELECT * FROM coupons_coupons ";
		$sql .= "WHERE `coupon_id`='{$aId}'";
	...
	function getLimit()
	{
	        $sql = "SELECT * FROM {$this->mPrefix}config where id=21";
	        return $this->mDb->getRow($sql);
	}
	
	function getCity($aId)
	{
		$sql  = "SELECT * FROM coupons_cities ";
		$sql .= "WHERE `cityid`='{$aId}'";
...
//echo $sql;
	...
	function getCommissionsByStatusUpdated($aUserId, $aStatus = '1', $aStart = 0, $aLimit = 0)
	{
		$sql  = "SELECT * FROM `{$this->mPrefix}sales` ";
		$sql .= "WHERE `approved` = '{$aStatus}' ";
		$sql .= "AND `aff_id` = '{$aUserId}' AND `fund_requested` = 0 ";
		$sql .= "AND `id` > 0 ";
		$sql .= $aLimit ? "LIMIT {$aStart}, {$aLimit}" : '';
//echo $sql;
		return $this->mDb->getAll($sql);
	...
	function getCommissionsByStat($aUserId, $aStatus = '2', $aStart = 0, $aLimit = 0)
	{
		$sql  = "SELECT * FROM `{$this->mPrefix}sales` ";
		$sql .= "WHERE `approved` = '{$aStatus}' ";
		$sql .= "AND `aff_id` = '{$aUserId}' ";
	...
	/* SELECT The sales based on the rasing fund */
	function getCommissionsRFByStat($aUserId, $aStatus = '2', $aStart = 0, $aLimit = 0)
	{
		$sql  = "SELECT * FROM `{$this->mPrefix}sales` ";
		$sql .= "WHERE `approved` = '{$aStatus}' ";
		$sql .= "AND `aff_id` = '{$aUserId}' ";
		$sql .= "AND `id` > 0 order by id ASC  ";
		$sql .= $aLimit ? "LIMIT {$aStart}, {$aLimit}" : ''; 
		//echo $sql;
		return $this->mDb->getAll($sql);
	}
	...
	function getCommissionsRFByStatUpdated($aUserId, $aStatus = '2', $aStart = 0, $aLimit = 0, $com)
	{
		...
		$sql .= "WHERE `approved` = '{$aStatus}' ";
		$sql .= "AND `aff_id` = '{$aUserId}' ";
		$sql .= "AND `id` > 0 AND `id` in ({$value}) order by id ASC  ";
		$sql .= $aLimit ? "LIMIT {$aStart}, {$aLimit}" : ''; 
		//echo $sql;
		return $this->mDb->getAll($sql);
	}
	...
	function getTransactionList($aAff)
	{
		$sql  = "SELECT SUM(payout) as transaction ";
		$sql .= "FROM `{$this->mPrefix}archived_sales` ";
		$sql .= "WHERE `aff_id` = '{$aAff['id']}'";
		
		return $this->mDb->getOne($sql);
	}
	
        function getSaleList($aAff)
	{
		$sql  = "SELECT COUNT(*) as salecount ";
		$sql .= "FROM `{$this->mPrefix}archived_sales` ";
		$sql .= "WHERE `aff_id` = '{$aAff['id']}'";
