...
//echo $sql;
	...
	*/
	...
	function getAccountsToBePaid($aStart, $aLimit)
	{
		...
		$sql .= "WHERE `approved` = '2' and `fund_requested`  = 1 ";
		$sql .= "GROUP BY `aff_id` ";
...
//echo $sql;
	...
	function getAccountsToRaisingFund($aStart, $aLimit)
	{
		global $gXpConfig;

		$sql  = "SELECT * from `{$this->mPrefix}fund_raising` ";
		$sql .= "WHERE `status` = '0' ";
		//$sql .= "GROUP BY `aff_id` ";
		$sql .= $aLimit ? "LIMIT {$aStart}, {$aLimit}" : '';
		//echo $sql;
	...
	function getAccountToRaisingFund($aId)
	{
		$sql  = "SELECT * from `{$this->mPrefix}fund_raising` ";
		$sql .= "WHERE `aff_id` = '{$aId}' AND `status`='0'";

		return $this->mDb->getRow($sql);
	}
	
	
	function getRaisingFund($aId)
	{
		$sql  = "SELECT * from `{$this->mPrefix}fund_raising` ";
		$sql .= "WHERE `aff_id` = '{$aId}' AND `status`='2'";
	...
	function getAccountToBeComm($aId)
	{
		$sql  = "SELECT SUM(payout) AS `Total`, COUNT(*) AS `Sales` from `{$this->mPrefix}sales` ";
		$sql .= "WHERE `aff_id` = '{$aId}' AND `approved`='2'";
		//echo $sql;
		return $this->mDb->getRow($sql);
	}
	...
	{
		$sql  = "SELECT * FROM `{$this->mPrefix}sales` ";
		$sql .= "WHERE `aff_id`='{$aId}' AND `approved`='2' AND `fund_requested`='1' ";
	...
	function archiveFRSales($aSale, $new_com, $aUid)
	{
		$sql  = "INSERT INTO `{$this->mPrefix}archived_sales` SET ";

		$total = count($aSale);
		$cnt = 1;
		$aSale['payout'] = $new_com; 
		foreach($aSale as $key=>$value)
		{
		       
			//$sql .= ($cnt == $total) ? "`{$key}` = '{$value}' " : "`{$key}` = '{$value}', ";
			if( $key != 'id' && $key != 'approved' && $key != 'tracking' )
			$sql .= "`{$key}` = '{$value}', ";
			$cnt++;$aSale['id']++;
		}
		$sql .= "`uid`='{$aUid}'";
	...
	function deleteSalesL($aId,$Limit)
	{
		$sql  = "DELETE FROM `{$this->mPrefix}sales` ";
	        $sql .= "WHERE `aff_id` = '{$aId}' AND `approved` = '2' LIMIT 1";
	...
	function updatefund_check($aId)
	{
		$sql  = "UPDATE `{$this->mPrefix}fund_raising` SET ";
		$sql .= "status = 2 ";
		$sql .= "WHERE `aff_id` = '{$aId}'";
	...
	function updateSales($new_com, $uid)
	{
		$sql  = "UPDATE `{$this->mPrefix}sales` SET ";
		$sql .= "payout  = $new_com ";
	        $sql .= "WHERE `aff_id` = '{$uid}' AND `approved` = '2' LIMIT 1"; 
	...
	{
	        $sql  = "SELECT * FROM `coupons_country` ";
		$sql .= "WHERE `status` = 'A' ";
