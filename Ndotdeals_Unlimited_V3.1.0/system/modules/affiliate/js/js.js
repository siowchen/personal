...
function displayGrouponAd(APIKEY, size, location, color1, showpreloader, pid, aid, title) {
	...
	APIURL = 'http://www.groupon.com/api/v1/deals.json?apikey='+APIKEY+'&callback=?';
	if (location.length) {
	...
	if (showpreloader) {
		jQuery('#grouponAdContainer').html('<div style="text-align: center; font: 10px arial normal; width: ' + sizeSplit[0] + '"><img width="64" height="64" src="http://www.groupon.com/images/groupon/widget/loader.gif" align="center" /><br>Loading Groupon</div>');
	}
	jQuery.getJSON(APIURL, function(data) {
		...
		switch (size) {
			...
			case '88.31':
				...
				var grouponAd = '<div id="grouponAdContents"><a title="' + thisTitle.replace(/\"/g, '&quot;') + '" href="' + cjprepend+data.deals[0].deal_url + '"><img src="http://www.groupon.com/images/groupon/widget/banner.88.31.png"></a></div>';
