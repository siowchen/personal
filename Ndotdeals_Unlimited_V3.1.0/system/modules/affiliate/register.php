...
if(!$login)
{
	if ($_POST['register'])
	{
		...
		$temp['city']		= htmlentities($tmp['city']);
	        $temp['country']	= htmlentities($tmp['country']);
		...
		$temp['paypal_email']		= htmlentities($tmp['aemail']);
		...
		/** check email **/
		if (!valid_email($_POST['aemail']))
		{
			$error = true;
			$msg .= "<li>{$gXpLang['error_email_incorrect']}</li>";
		}
		elseif ($gXpDb->checkAffiliateByPEmail($temp['paypal_email']))
		{
			$error = true;
			$msg .= "<li>{$gXpLang['editor_email_exists']}</li>";
		}
