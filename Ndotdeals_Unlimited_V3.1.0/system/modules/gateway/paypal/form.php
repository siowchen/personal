...
<!-- Not allowing the specialchars in Quantity field--> 
 <script type="text/javascript">
	...
	//==================================
        $("#credit_quantity").numeric({allow:""});
        $("#ref_amt2").numeric({allow:"."});
   });
   
</script>
  <form method="POST" id="credit_card_form" action="<?php echo DOCROOT; ?>system/modules/gateway/paypal/DoDirectPaymentReceipt.php" name="DoDirectPaymentForm">
		<div style="float:left;clear:both;width:600px; margin-left:14px;">
		<?php if($available_amt > 0) { ?>
	                <p class="fl clr" style="width:600px;"><strong style="color:#666;float:left;clear:both;" class="p5"><?php echo $language["your_ref_bal"]; ?></strong> <?php echo CURRENCY; ?><span id="total_ref"><?php echo $available_amt; ?></span><strong style="color:#666;" class="p5">.<?php echo $language["use_int"]; ?></strong></p>
		<?php } else { 
				$available_amt = 0;
		?>
		...
		<table cellpadding="5" align="left" class="fl width930 clr paypal_form " border="0">
		...
		<td></td>
		<td><?php echo $language['total']; ?></label></td>
		</tr>
		
		<tr>
		<td style="font:normal 12px arial!important;color:#666!important;width:600px;float:left; clear:both;" id="face">
		<h5 style="font:normal 12px arial!important;color:#666!important;width:600px;clear:both;" id="face"><?php echo $language['payment_for'].' '.ucfirst($couponname); ?></h5>
		</td>
		<td>
		<input type="text" style="width:48px;" name="quantity" id="credit_quantity" title="Enter quantity in numeric" class="digits required fl lh25" maxlength="5" value="1" onblur="calculate_credit_amt(this.value,<?php echo $total_payable_amount; ?>)" />
		</td>
		...
		<td><label class="fl lh30"><?php echo CURRENCY; ?></label><input type="text" class="fl lh25" name="ref_amt2" id="ref_amt2" style="width:48px;" maxlength="5" value="0" onblur="calculate_referral1(this.value,<?php echo $total_payable_amount; ?>)"></td>
		...
		<td><h5 style="font:normal 12px arial!important;color:#666;">=</h5></td>
		<td>
		<h5 style="font:normal 12px arial!important;color:#666;"><?php echo CURRENCY; ?><span id="total_credit_amt"><?php echo $total_payable_amount; ?></span>
		</h5>
		</td>
		</tr>
	...
	<tr>
		...
		<td><?php echo $language['state']; ?>:
		</td>
	</tr>
	
	<tr>	
		<td><input type="text" size="19" maxlength="19" name="creditCardNumber" class="required" title="<?php echo $language['card_no_req']; ?>"></td>
		<td><input type="text" size="2" maxlength="2" class="required" title="<?php echo $language['enter_your_state'];?>" name="state" value=""  />
        	<span style="width:200px;font:italic 12px arial;clear:both;">(2 <?php echo $language['characters']; ?>)</span>
        </td>
	</tr>
	...
	<tr>
		...
		<td>
		<input type="text" size="10" minlength="5" class="required" title="<?php echo $language['enter_zip_code']; ?>" maxlength="10" name="zip" value="">
		<br />
		<span style="width:200px;font:italic 12px arial;clear:both;">(5 to 9 <?php echo $language['digits']; ?>)</span>
		</td>
	</tr>
	...
	<tr>
		...
		<td><input type="text" size="2" maxlength="2" name="countrycode" class="width50 required" value="<?php //echo $getdetails->countrycode;?>" title="<?php echo $language['enter_your_country'];?>">
		<br />
	...
	<span class="submit fl clr ml15"><input class="bnone" type="Submit" value="<?php echo $language['pay_now']; ?>" ></span>
