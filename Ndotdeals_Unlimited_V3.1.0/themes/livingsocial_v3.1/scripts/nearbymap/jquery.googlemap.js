...
function imLatLngs(lat, lng, place, bus_name, phone, desc, images, deal_info, link_id, show_id) {
	this.lat = lat;
	this.lng = lng;
	this.place = place;
	...
	this.show_id = show_id; // Deal onchange value 
}
...
$.googleMaps = function(obj, opts) {
	...
	function getAdrLocation(cnt) {
		...
		var link_id = aRequestedAdress[cnt].link_id;
                var show_id = aRequestedAdress[cnt].show_id;
...
alert(adr);
				...
				aLatLng[cnt] = new imLatLngs(place.Point.coordinates[1], place.Point.coordinates[0], place, bName, phone, desc, img, deal_info, link_id, show_id);
	...
	function mapAddress() {
		...
		$.each(aLatLng, function(i, itm){
			...
			var marker = createMarker(point, i, itm.place, itm.bus_name, itm.phone, itm.desc, itm.images, itm.deal_info, itm.link_id, itm.show_id);
	...
	function createMarker(point, index, place, bus_name, phone, desc, images, deal_info, link_id, show_id) {
		...
		GEvent.addListener(mrkr, "click", function() {
			...
			var w = document.getElementById("nexttest").innerHTML;
			t=index;
			$(".box").toggle("hide");
			$("#slider_cont").css("margin-left",index*(-696));
			var addr;
			...
			if (desc) {
		        ...
		        if(document.getElementById(link_id).innerHTML != '')
		        {
		                //document.getElementById(link_id).innerHTML = deal_info
		        }
		...
		$("#prevBtn,#nextBtn").click(function(){
		    ...
		    imGMapObj.panDirection(0, +2);
	       //alert(bounds.getCenter());
                
		//document.getElementById("nexttest").innerHTML = 'nextBtn';
		//alert(next);
		//imGMapObj.panTo(point);
		//$(prevbtn, nextbtn).click(function(){
		var addr;
		if(document.getElementById("nexttest").innerHTML != '')
		{
		        var index_value= document.getElementById("nexttest").innerHTML;
		        if(link_id == 'address'+index_value)
		        {
		                if ((place.AddressDetails.Country.AdministrativeArea.SubAdministrativeArea.Locality.Thoroughfare) && (place.AddressDetails.Country.AdministrativeArea.SubAdministrativeArea) && (place.AddressDetails.Country.AdministrativeArea.AdministrativeAreaName)) {
				var str = place.AddressDetails.Country.AdministrativeArea.SubAdministrativeArea.Locality.Thoroughfare.ThoroughfareName;
				var cty = place.AddressDetails.Country.AdministrativeArea.SubAdministrativeArea.Locality.LocalityName;
